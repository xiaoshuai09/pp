...
def get_intensity(seq, n_seqs, n_t=100, t0=0, T=None):
    ...
    return pdf, np.arange(t0, T+delta_t, delta_t)[:-1]
...
def intensityplot4seqs(learner_seqs, expert_seqs, T, n_t=20, t0=0,
...
if __name__ == "__main__":
    ...
    expert_seqs = file2sequence(data)
    
    #learner_seqs = np.loadtxt("data/wgan_solver_hawkes_gaussian_0_0.1.txt", delimiter="\t")
    name_list = ["wgan_solver_{}_0_0.1","wgan_solver_{}_0_0.3","wgan_solver_{}_0_0.9","wgan_solver_{}_0_3.0",\
                  "hawkes_solver_{}","rmtpp_{}_2000_0","gaussian_solver_{}_2000_1","selfcorrecting_solver_{}"]
    name_list = [item.format(data) for item in name_list]

    for name in name_list:
        learner_seqs = file2sequence(name)
        learner_seqs = learner_seqs[:100]
        expert_seqs = expert_seqs[:100]

        # Plot 1: Q-Q plot
        #print >> sys.stderr, "[%s] Plotting Q-Q plot..." % arrow.now()
        if have_intensity:
            qqplot4intdiff(learner_seqs, expert_seqs, intensity, T=T_max, file_path="results/{}_qqplot4intdiff.png".format(name))

        # Plot 2: Intensity plot
        #print >> sys.stderr, "[%s] Plotting Intensity plot..." % arrow.now()
        intensityplot4seqs(learner_seqs, expert_seqs, T=T_max, file_path="results/{}_intensityplot4seqs.png".format(name))
