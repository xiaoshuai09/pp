...
##############################################################################
# parameters
MODE = 'wgan-lp' # wgan or wgan-gp, wgan-lp
...
PRE_TRAIN = True
COST_ALL = True
G_DIFF = True # check the logic for fake and real
D_DIFF = True
MARK = False
ITERATION = 0
...
###############################################################################
# define model
#fuse rnn output into input or not
...
if MARK:
    fake_data_discrete = tf.argmax(fake_data[:,:,1:], axis=2) # Attention, output dimension 3 or 2
...
if MODE == 'wgan-gp':
    ...
    #Error no interpolate
    interpolates = alpha*real_data + ((1-alpha)*fake_data)
    disc_interpolates = discriminator(interpolates,fake_seqlen,lower_triangular_ones,scope_reuse=True)
    ...
    if MARK:
        ...
        length_ = tf.minimum(tf.shape(real_data)[1],tf.shape(fake_data)[1])
        lipschtiz_divergence = tf.abs(D_real-D_fake)/tf.sqrt(tf.reduce_sum(tf.square(real_data[:,:length_,:]-fake_data[:,:length_,:]), axis=[1,2])+0.00001)
        #lipschtiz_divergence = tf.abs(D_real-D_fake)/tf.sqrt(tf.reduce_sum(tf.square(tf.cumsum(real_data[:,:length_,:],axis=1)-tf.cumsum(fake_data[:,:length_,:],axis=1)), axis=[1,2])+0.00001)
    ...
    disc_train_op = tf.train.RMSPropOptimizer(learning_rate=5e-5).minimize(D_loss, var_list=discriminator_variables)
    gen_train_op = tf.train.RMSPropOptimizer(learning_rate=10e-5).minimize(G_loss, var_list=generator_variables)
    # modify generator

    # Build an op to do the weight clipping
    clip_D = [p.assign(tf.clip_by_value(p, -0.01, 0.01)) for p in discriminator_variables]
...
saved_file = "wgan_{}_{}_{}".format(DATA,ITERATION,LAMBDA_LP)
...
for it in range(ITERS):
    ...
    if it%4999==0 or stop_indicator: 
        sequences_generator = []
        for _ in range(int(2000/BATCH_SIZE)):
            fake_batch = fake_iterator.next_batch(BATCH_SIZE)
            sequences_gen = sess.run(fake_data,feed_dict={Z:fake_batch[0], fake_seqlen:fake_batch[1]})
            ...
            sequences_gen = sequence_filter(sequences_gen,fake_batch[1]) # remove padding tokens
            sequences_generator +=sequences_gen
        sequence2file(sequences_generator, 'wgan_solver_{}_{}_{}'.format(DATA,ITERATION,LAMBDA_LP))
